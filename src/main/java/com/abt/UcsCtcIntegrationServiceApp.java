...
public class UcsCtcIntegrationServiceApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Behavior<NotUsed> rootBehavior = Behaviors.setup(context -> {
            ActorRef<UcsCtcIntegrationRegistry.Command> userRegistryActor =
                context.spawn(UcsCtcIntegrationRegistry.create(), "UcsCtcIntegration");

            UcsCtcIntegrationRoutes ucsCtcIntegrationRoutes = new UcsCtcIntegrationRoutes(context.getSystem(), userRegistryActor);
            startHttpServer(ucsCtcIntegrationRoutes.ucsIntegrationRoutes(), context.getSystem());

            return Behaviors.empty();
        });

        // boot up server using the route as defined below
        ActorSystem.create(rootBehavior, "UcsCtcIntegrationServiceServer");
        //#server-bootstrapping
    }
